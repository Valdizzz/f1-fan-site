...
/**
 * Основной компонент приложения для фанатов Mercedes F1.
 * Позволяет загружать селфи и генерировать фото с Джорджем Расселом,
 * а также просматривать историю достижений команды.
 */
const App = () => {
  ...
  // Безопасное получение API ключа из переменных окружения (с поддержкой старых стандартов)
  const getApiKey = () => {
    try {
  ...
  // Список исторических достижений Mercedes в F1
  const achievements = [
  ...
  const generateFanPhoto = async () => {
    if (!apiKey) {
      setError("Ошибка конфигурации: API ключ не найден в Secrets репозитория.");
      return;
    }
    if (!userPhotoBase64) {
      setError("Пожалуйста, сначала загрузите свое селфи!");
      return;
    }
    setIsGenerating(true);
    setError(null);
    
    // Промпт для создания реалистичного фото с Джорджем Расселом
    const prompt = "A high-quality realistic photo where the woman from the attached portrait is standing next to F1 driver George Russell. They are both wearing black Mercedes-AMG Petronas F1 team shirts, smiling at the camera in a professional paddock setting. Maintain her facial features accurately.";
    ...
    try {
      ...
      setError("Не удалось создать фото. Проверьте настройки API ключа и попробуйте еще раз.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#00A19B]">
        ...
        <div className="absolute inset-0 z-0">
          {imageUrl ? (
            <img 
              ...
              style={{ transform: showUI ? 'scale(1)' : 'scale(1.05)' }} 
              alt="Generated fan moment"
            />
          ) : (
        ...
        {imageGenerated && (
          <div className="absolute top-6 right-6 z-50 flex gap-3">
             <button 
              ...
              className="p-3 bg-black/40 backdrop-blur-md border border-white/10 rounded-full hover:bg-white/20 transition-all"
             >
              <RefreshCw size={20} />
             </button>
             <button 
            ...
            {!imageGenerated ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                <div className="space-y-6">
                    <h1 className="text-5xl md:text-7xl font-black italic leading-none uppercase">
                      Mercedes <br /><span className="text-[#00A19B]">Fan Lab</span>
                    </h1>
                    <p className="text-gray-400 text-lg">
                      Загрузи свое лучшее селфи и окажись в боксах Mercedes вместе с Джорджем Расселом.
                    </p>
                </div>
                <div className="bg-white/5 border border-white/10 p-8 rounded-3xl backdrop-blur-xl">
                    {!userPhoto ? (
                        ...
                        <div className="relative">
                            <img src={userPhoto} className="h-64 w-full object-cover rounded-2xl border border-[#00A19B]/50" alt="Selfie preview" />
                            <button 
                              onClick={() => { setUserPhoto(null); setUserPhotoBase64(null); }} 
                              className="absolute top-2 right-2 p-2 bg-black/60 rounded-full hover:bg-red-500 transition-colors"
                            >
                              <X size={16} />
                            </button>
                        </div>
                    ...
                    <button 
                      ...
                      className="w-full mt-6 py-5 bg-[#00A19B] text-white font-bold rounded-2xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-30 flex justify-center items-center gap-3 uppercase tracking-wider"
                    >
                        {isGenerating ? 'Создаем магию...' : 'Создать фото'} <Camera size={22} />
                    </button>
                    {error && <p className="text-red-400 text-center mt-4 text-sm font-medium italic">{error}</p>}
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <h1 className="text-6xl md:text-9xl font-black italic uppercase leading-none">
                  Perfect <br /><span className="text-[#00A19B]">Teammate.</span>
                </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
            <div className="sticky top-32 h-fit">
              <h2 className="text-5xl md:text-7xl font-black italic uppercase">
                История <br /><span className="text-[#00A19B]">Триумфа</span>
              </h2>
              <p className="text-gray-500 mt-6 text-lg">
                Путь легендарных «Серебряных стрел» сквозь десятилетия к вершинам автоспорта.
              </p>
            </div>
            <div className="space-y-24">
              {achievements.map((item, i) => (
                <div key={i} className="group relative pl-8 border-l-2 border-gray-900 hover:border-[#00A19B] transition-all">
                  <div className="absolute -left-[9px] top-0 w-4 h-4 rounded-full bg-gray-900 border-2 border-gray-800 group-hover:bg-[#00A19B] group-hover:border-[#00A19B] transition-colors" />
      ...
      <footer className="py-20 border-t border-gray-900 text-center">
        <p className="text-gray-600 uppercase tracking-[0.4em] text-[10px]">Made for Mercedes-AMG Petronas F1 Fans</p>
      </footer>
    </div>
  );
};
